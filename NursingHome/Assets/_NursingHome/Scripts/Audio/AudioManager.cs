...
namespace WoodlandGames.Audio
{
    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        Dictionary<ulong, ClipWithCollection> _loopingParams = new Dictionary<ulong, ClipWithCollection>();
        List<ulong> _loopingSounds = new List<ulong>();
        ...
        {
            ulong currentID = id;
            if (loopingSound && _loopingParams.ContainsKey(id))
            {
                currentID = _loopingParams[id].LoopID;
            }

            AudioPoolItem activeSound;
            if (_activePool.TryGetValue(currentID, out activeSound))
            {
                ...
                {
                    activeSound.VolumeRoutine = SetSoundVolumeRoutine(currentID, activeSound, volume, fadeTime);
        ...
        {
            if (_activePool.ContainsKey(id) || _loopingSounds.Contains(id))
            {
                return true;
            }
            ...
            int priority = 128, float startTime = 0.0f, bool ignoreListenerPause = false, float pitch = 1, bool randomPitch = false, float pitchRandomValue = 0)
        {
            ...
            if (!_listnerPosition)
            {
                _listnerPosition = FindObjectOfType<AudioListener>(true).transform;
            }
            ...
            for (int i = 0; i < _pool.Count; i++)
            {
                ...
                else if (poolItem.Unimportance > leastImportanceValue && !poolItem.AudioSource.loop) // Look for a non-looping, minimal importance source
                {
        ...
        private void SetSoundAsLooping(ulong id)
        {
            if (id == 0) return;
            ...
            if (_activePool.TryGetValue(id, out activeSound))
            {
                activeSound.AudioSource.loop = true;

                _loopingSounds.Add(id);
